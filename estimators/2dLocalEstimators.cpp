...
/**
 ...
 @section Doc2dLocalEstimators_sec 2dLocalEstimators
...
int main( int argc, char** argv )
{
  ...
  auto kernelradiusOpt = app.add_option("--kernelradius,-K", radius, "Radius of the convolution kernel (Integral invariants estimators)");
  auto alphaOpt = app.add_option("--alpha", alpha, "Alpha parameter for Integral Invariant computation");
  ...
  auto smallradiusOpt = app.add_option("--smallradius,-r", smallradius, "Small radius of the shape");
  auto varsmallradiusOpt = app.add_option("--varsmallradius,-v", varsmallradius, "Variable small radius of the shape" );
  auto kOpt = app.add_option("-k", k, "Number of branches or corners the shape (default 3)" );
  auto phiOpt = app.add_option("--phi", phi, "Phase of the shape (in radian)" );
  auto widthOpt = app.add_option("--width,-w", width, "Width of the shape" );
  auto powerOpt = app.add_option("--power,-p", power, "Power of the metric" );
  app.add_option("--center_x,-x", cx, "x-coordinate of the shape center" );
  app.add_option("--center_y,-y", cy, "y-coordinate of the shape center" );
  app.add_option("--gridstep,-g", h, "Gridstep for the digitization" );
  ...
  app.add_option("--exportShape,-E", outShape, "Exports the contour of the source shape as a sequence of discrete points (.sdp)");
  app.add_option("--lambda", lambda, "Use the shape to get a better approximation of the surface (optional)");
