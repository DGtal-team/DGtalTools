...
int main( int argc, char** argv )
{
  ...
  std::string imageFileName = vm["pgmImage"].as<std::string>();
  std::string imageOutput = vm["output"].as<std::string>();
  Image imageSRC = GenericReader<Image>::import( imageFileName ); 
  ...
  trace.error() <<  "lower bound " << image.domain().lowerBound() << std::endl;
  trace.error() <<  "upper bound " << image.domain().upperBound() << std::endl;
  ...
  for( unsigned int i= 0; i<=255; i+=step){
    ...
    trace.info()<< "preocessing step " << i << std::endl;
    std::vector< std::vector< Point >  >  vectContours;
    Surfaces<Z2i::KSpace>::extractAllPointContours4C( vectContours,
                                                      ks, predicate, sAdj );  
    std::vector< std::vector< SCell >  >  vectContoursSCell;
    Surfaces<Z2i::KSpace>::extractAll2DSCellContours(  vectContoursSCell,
                                                       ks, sAdj , predicate );
    for(unsigned int k=0; k< vectContours.size(); k++){
      
      std::vector<Z2i::Point> aContour = vectContours.at(k);
      ...
      std::vector<LibBoard::Point> aContourSampled;
      if(vm.count("samplingFrechet")){
  ...
  std::string extension = imageOutput.substr(imageOutput.find_last_of(".") + 1);
  if(extension=="eps"){
