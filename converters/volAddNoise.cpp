...
int main( int argc, char** argv )
{
  ...
  general_opt.add_options()
    ...
    ("input,i", po::value<std::string>(), "input image file name (any 3D image format accepted by DGtal::GenericReader)")
    ("output,o", po::value<std::string>(), "output image file name (any 3D image format accepted by DGtal::GenericWriter)")
  ...
  KanungoNoise<Binarizer, Z3i::Domain> kanungo(predicate, image.domain(), noise);
  
  MyImage result(image.domain());
  for(Z3i::Domain::ConstIterator it = image.domain().begin(), itend = image.domain().end(); it!= itend; ++it)
  {
    if (kanungo(*it))
