    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        * Constructor from Khalimsky space, which specifies the domain of calculus
        * and te step size for considered image
        */
        void init( Clone<KSpace> K, double h );
...
*/
        ...
        /**
        * Adds an input 2-form of size (N x N) from an image of size (N/2 x N/2).
        *
        * @param image restored image at step n-1 for multiresolution option.
        *
        * @param f any functor associated a scalar to an image value.
        *
        * @note For a grey-level image stored with values `unsigned char`,
        * should be called as
        * @code
        * AT.setUFromImage( image, [] (unsigned char c ) { return (double) c / 255.0; } );
        * @endcode
        *
        *
        * @tparam Image previous Image type.
        * @tparam Function any function type ( typename Image::Value ) -> Scalar.
        */
        template <typename Image, typename Function>
        void setUFromImage(const Image& image, const Function& f);

        /**
        * Sets approximation \a v to be equal to the previous result for
        * discontinuities of restored image u
        */
        template <typename Image, typename Function>
        void setVFromImage(const Image& image, const Function& f);
    ...
    public:
        ...
        /// metric tensor for 0-forms
        PrimalIdentity0       G0;
        /// metric tensor for 1-forms
        PrimalIdentity1       G1;
        /// metric tensor for 2-forms
        PrimalIdentity2       G2;
    ...
    protected:
        ...
        double epsilon;
        /// Step size in order to include metric
        double H;
        ///  Parameters included the step size h
        double alpha_h;
        double lambda_h;
        double epsilon_h;
